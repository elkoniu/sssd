...
static void ipa_subdomain_account_got_override(struct tevent_req *subreq)
{
    ...
    if (state->override_attrs != NULL) {
        BE_REQ_DEBUG(SSSDBG_TRACE_ALL, req, "Processing override.\n");
        ...
        if (state->mapped_attrs != NULL) {
            ...
            if (ret != EOK) {
                BE_REQ_DEBUG(SSSDBG_OP_FAILURE, req, "sysdb_remove_mapped_data failed, "
                                                       ...
                                                       struct dp_id_data *ar)
{
    ...
    if (subreq == NULL) {
        BE_REQ_DEBUG(SSSDBG_OP_FAILURE, req, "ipa_get_*_acct_send failed.\n");
        return ENOMEM;
    }
...
static void ipa_subdomain_account_done(struct tevent_req *subreq)
{
    ...
    talloc_zfree(subreq);
    if (ret != EOK) {
        BE_REQ_DEBUG(SSSDBG_OP_FAILURE, req, "ipa_get_*_acct request failed: [%d]: %s.\n",
    ...
    if (state->mapped_attrs != NULL) {
        ...
        if (ret == EOK) {
            ...
            BE_REQ_DEBUG(SSSDBG_TRACE_ALL, req, "No cached object found, cannot add "
    ...
    switch (state->entry_type) {
        ...
        default:
            ...
            if (state->entry_type > BE_REQ__LAST) {
                BE_REQ_DEBUG(SSSDBG_OP_FAILURE, req, "Invalid sub-domain request type %d.\n",
                      state->entry_type);
            } else {
                BE_REQ_DEBUG(SSSDBG_TRACE_FUNC, req, "Unhandled sub-domain request type %d.\n",
                      state->entry_type);
            }
    }
...
static void ipa_get_subdom_acct_connected(struct tevent_req *subreq)
{
    ...
    if (state->entry_type == BE_REQ_INITGROUPS) {
        ...
        if (sdap_is_extension_supported(sdap_id_op_handle(state->op),
            ...
            BE_REQ_DEBUG(SSSDBG_TRACE_FUNC, req, "Initgroups requests are not handled " \
    ...
    switch (state->filter_type) {
        ...
        case BE_FILTER_CERT:
            ...
            break;
        default:
            BE_REQ_DEBUG(SSSDBG_OP_FAILURE, req, "Invalid sub-domain filter type.\n");
            state->dp_error = dp_error;
            tevent_req_error(req, EINVAL);
            return;
    }
                     ...
                     struct dp_id_data *ar)
{
    ...
    if (clist == NULL) {
        BE_REQ_DEBUG(SSSDBG_OP_FAILURE, req, "Cannot generate AD connection list!\n");
        ret = ENOMEM;
        goto fail;
    }
...
ipa_get_ad_acct_ad_part_done(struct tevent_req *subreq)
{
    ...
    if (ret == ERR_SUBDOM_INACTIVE) {
        ...
        BE_REQ_DEBUG(SSSDBG_OP_FAILURE, req, "AD lookup failed: %d\n", ret);
        tevent_req_error(req, ret);
        return;
    }
    ...
    if (state->override_attrs == NULL) {
        ...
        if (sid == NULL) {
            BE_REQ_DEBUG(SSSDBG_CRIT_FAILURE, req, "Cannot find a SID.\n");
            ret = EINVAL;
            goto fail;
        }
...
ipa_get_ad_override_done(struct tevent_req *subreq)
{
    ...
    ret = ipa_get_ad_apply_override_step(req);
    if (ret != EOK) {
        BE_REQ_DEBUG(SSSDBG_OP_FAILURE, req, "ipa_get_ad_apply_override_step failed.\n");
        goto fail;
    }
...
static void ipa_check_ghost_members_done(struct tevent_req *subreq)
{
    ...
    talloc_zfree(subreq);
    if (ret != EOK) {
        BE_REQ_DEBUG(SSSDBG_OP_FAILURE, req, "ipa_resolve_user_list request failed [%d]\n",
                                  ret);
        tevent_req_error(req, ret);
        return;
    }
...
static errno_t ipa_get_ad_apply_override_step(struct tevent_req *req)
{
    ...
    if (entry_type == BE_REQ_GROUP) {
        ...
        if (ret == EOK) {
            ...
            BE_REQ_DEBUG(SSSDBG_OP_FAILURE, req, "ipa_check_ghost_members failed.\n");
            return ret;
        }
    }
    ...
    if ((entry_type == BE_REQ_USER && state->ar->filter_type == BE_FILTER_IDNUM)
        ...
        obj_name = ldb_msg_find_attr_as_string(state->obj_msg, SYSDB_NAME,
                                               NULL);
        if (obj_name == NULL) {
            BE_REQ_DEBUG(SSSDBG_CRIT_FAILURE, req, "Cached object has no name.\n");
            return EINVAL;
        }
    ...
    if (ret != EOK) {
        BE_REQ_DEBUG(SSSDBG_OP_FAILURE, req, "ipa_get_ad_groups_without_orig failed.\n");
        return ret;
    }
    ...
    if (groups != NULL) {
        ...
        if (subreq == NULL) {
            BE_REQ_DEBUG(SSSDBG_OP_FAILURE, req, "ipa_initgr_get_overrides_send failed.\n");
            return ENOMEM;
        }
    ...
    if (ret != EOK) {
        BE_REQ_DEBUG(SSSDBG_OP_FAILURE, req, "ipa_get_ad_ipa_membership_step failed.\n");
        return ret;
    }
...
static errno_t ipa_get_ad_ipa_membership_step(struct tevent_req *req)
{
    ...
    subreq = ipa_get_ad_memberships_send(state, state->ev, state->ar,
                                         ...
                                         state->ipa_ctx->server_mode->realm);
    if (subreq == NULL) {
        BE_REQ_DEBUG(SSSDBG_OP_FAILURE, req, "ipa_get_ad_memberships_send failed.\n");
        return ENOMEM;
    }
...
ipa_get_ad_acct_done(struct tevent_req *subreq)
{
    ...
    talloc_zfree(subreq);
    if (ret != EOK) {
        BE_REQ_DEBUG(SSSDBG_OP_FAILURE, req, "IPA external groups lookup failed: %d\n",
                                  ret);
        tevent_req_error(req, ret);
        return;
                     ...
                     struct dp_id_data *ar)
{
    ...
    if (state->obj_dom == NULL) {
        BE_REQ_DEBUG(SSSDBG_OP_FAILURE, req, "Domain not found\n");
...
static int ipa_srv_ad_acct_lookup_step(struct tevent_req *req)
{
    ...
    BE_REQ_DEBUG(SSSDBG_TRACE_FUNC, req, "Looking up AD account\n");
...
static void ipa_srv_ad_acct_retried(struct tevent_req *subreq)
{
    ...
    BE_REQ_DEBUG(SSSDBG_TRACE_FUNC, req, "Subdomain re-set, will retry lookup\n");
    ad_id_ctx = ipa_get_ad_id_ctx(state->ipa_ctx, state->obj_dom);
    if (ad_id_ctx == NULL || ad_id_ctx->ad_options == NULL) {
        BE_REQ_DEBUG(SSSDBG_CRIT_FAILURE, req, "No AD ID ctx or no ID CTX options?\n");
        state->dp_error = DP_ERR_FATAL;
        tevent_req_error(req, EINVAL);
        return;
    }
